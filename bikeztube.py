...
def add_bike():
    ...
    conn = sqlite3.connect('bikes.db')
    c = conn.cursor()
    c.execute("INSERT INTO bikes VALUES (:name, :phone, :bike, :id, :date , :work, :total)",
            {
                'name':name_entry.get(),
...
def query_database():
    ...
    for record in records:
        if count % 2 ==0:
            my_tree.insert(parent='', index='end', iid=count, text='', values=(record[1], record[2], record[3], record[0], record[5], record[6], record[7]), tags=('evenrow',))
        else:
            my_tree.insert(parent='', index='end', iid=count, text='', values=(record[1], record[2], record[3], record[0], record[5], record[6], record[7]), tags=('oddrow',))
        #increment counter
        count +=1
...
def select_record(e):
    ...
    try:
        ...
        work_entry.insert('1.0', values[5])
...
my_tree['columns'] = ('Name', 'Phone', 'Bike', 'ID', 'Date', 'Work', 'Total')
...
ser_combo = ttk.Combobox(ser_frame, width=15, values =comboValuesServices, state='readonly', font=('Verdana',10))
...
parts_combo = ttk.Combobox(parts_frame, width=15, values =comboValuesParts, state='readonly', font=('Verdana',10))
